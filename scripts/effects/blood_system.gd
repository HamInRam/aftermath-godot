...
func pressure_wash_pixel_water(world_position: Vector2, brush_radius: float, power: int, flow_direction := Vector2.ZERO) -> bool:
	var cleaned := false
	if is_instance_valid(ground_canvas): cleaned = ground_canvas.pressure_wash_at(world_position, brush_radius, power, flow_direction) or cleaned
	if is_instance_valid(wall_canvas): cleaned = wall_canvas.pressure_wash_at(world_position, brush_radius * 0.72, power, flow_direction) or cleaned
	return cleaned
