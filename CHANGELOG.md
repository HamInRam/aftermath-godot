...
- Fixed close-contact shots spawning behind the target when a long barrel passed through its body. One barrel query per trigger pull now selects a safe physical projectile origin while flashes/casings stay at the visual muzzle; the existing solid-wall no-fire guard remains.
- Enemy ballistic hit zones use the forward shot path through the body rather than mistaking the collision circle's rear/side entry for a limb. Legacy point-based effects and player damage rules remain unchanged.
- Prevented the generic stagger from overwriting a same-frame weapon-specific joint impulse. Preserved AI stagger timing, movement, bounded corpse travel and wall collision.
- Removed duplicate pellet-density reductions from entry/exit blood geometry and transient mist. Kept the Mossberg's nine 18-damage pellets, armour, falloff and the paid enhanced-round ledger unchanged.
- Fixed the blood texture upload queue casting freed chunks after siphoning. Same-frame new blood now replaces a retiring chunk instead of being lost with it.
- Added real-shot contact/range/armour regression, 64 centre-line angle checks, an enhanced nine-pellet/two-target penetration ledger test, and a real-renderer production shotgun capture fixture.

## [Unreleased] - The Descent Presentation Rebuild

- Replaced world materials, furniture and landmarks with directly authored neutral pixel art, continuous black-core/white-edge walls and immediate broken-window paint updates across twelve venues.
- Redrew all six live character roles, matched corpse clothing and missing-module anatomy, and removed the obsolete displaced rectangular corpse shadow.
- Rebuilt eight firearm-class silhouettes, staged recoil/reload/feed/chamber presentation and separated all visual gun motion from ballistic aiming. Mouse aiming resolves instantly; angular/movement dispersion is restrained.
- Fixed automatic cadence quantization, per-pellet repeated shoulder recoil and shell-casing initialization at the wrong origin. Directional camera kicks and non-extending hit stop preserve fast combat readability.
- Replaced the title and HUD with a native-pixel mansion composition, square controls and separate ammunition/vitality/Focus/blood corners. Added actual label elision, short execution keycaps and matching reticle paint/cache quantization.
- Removed active hue-cycle input and obsolete scene-cost combat notifications. Enhanced muzzle color now follows the paid blood transaction without making a second resource withdrawal.
- Added four CI regression scenes for world layouts/art, actor anatomy, gun presentation and interface bounds; expanded settings assertions for capped flashes and bounded hit stop.

## [Unreleased] - Six-floor Descent Integration

- Added an explicit six-floor run session with a seeded no-repeat venue deck, exact inventory/health/armor/blood/Focus inheritance, entry checkpoints, counted death retries, cumulative run statistics and a final victory report. Duplicate clear events cannot award a floor twice.
- Actual room geometry is twice as wide and tall at unchanged actor, tile and door scale. Expanded the authored formation library to 48 and standard encounters to 16–20 enemies. Fixed an After Hours ammunition anchor inside the enlarged door frame.
- Empty-magazine left click reloads; completely depleted left click dry-fires. Independent Q throw uses swept collision, clear spin, damped bounce and recoverable ammunition/attachments. Fixed duplicate pickup, throwing ammunition duplication, canceled reload completion and magazine-capacity losses.
- Matched severed anatomy to corpse wound states, rendered bisection in ragdolls, gave detached limbs real colliders and continuous angular momentum, and added post-constraint wall checks. Persistent anatomy no longer disappears when the cosmetic debris budget fills.
- Normalized prop impact energy rather than treating actor HP damage as raw physical impulse. Removed active restoration ghosts and debris cleanup registrations. Retained neutral-gray fragments and finite blood-only enemy impact sources.
- Removed 444 lines of unreachable cleaning/extraction handlers from the active level controller, plus their resource imports and transient state. Kept serialized legacy fields/save data for compatibility.
- Added fixed terminal actions and scrollable controls help. Run reports no longer display cleanup contracts, mastery paragraphs or a reward-farming retry action.
- Fixed doorway hit wake-up: bullets, thrown weapons and doors engage only the victim's room, existing encounters stay engaged, and separate rooms clear independently without duplicate floor completion.
- Blocked repeated Enter during scene fades from skipping floors or replacing a run. Floor advancement requires its current clear; completion must be sequential. Report actions lock on acceptance.
- Verified menus and gameplay in a real 1920×1080 renderer; removed a transient light-texture atlas error during native-pixel texture initialization.
- Added three regression scenes for noir impact physics, run sessions and actual scene reload/floor transfer. Added a bounded sequential regression runner and optional real-renderer viewport review.

## [Unreleased] - Roguelike Run & Authored Room Library

- Reframed combat around a faster OTXO-influenced arcade contract: the reticle now occupies a crisp 48px canvas with seven-pixel aiming arms, controller magnetism reaches farther, and movement/rapid-turn dispersion is heavily reduced while each firearm retains its base handling identity.
- Completely removed the room-clear three-choice modal, its pause path and its scene/script. Clearing a chamber now gives a brief monochrome acknowledgement and immediately restores traversal; future build choices are reserved for a dedicated safe intermission.
- Raised airborne blood, lethal mist, grayscale tissue debris and weapon-scaled dismemberment substantially while retaining a bounded sparse ground-blood budget. Ordinary firearms can sever one limb, high-burden weapons two and extreme shotgun/anti-materiel impacts three without allowing empowered blood to duplicate its resource cost.
- Enforced a final-frame black/white/gray/crimson shader contract: legacy cyan, violet, gold and venue hues collapse to luminance, ordinary projectile cores/trails are white/gray and only blood, blood-powered rounds and lethal crisis signals retain crimson.
- Widened the combat read from roughly 237x133 to 271x153 world pixels, expanded hostile formations into the full room footprint, lengthened patrol lanes and moved exactly two destructible anchors toward the perimeter. Critical ammo and doorway approach cells now have generated-cover exclusion zones.
- Replaced rigid siphon rays with as many as eighteen staggered, curved one-pixel blood motes. Each packet only paints its previous-to-current motion segment as it accelerates into the actor, eliminating the straight line attached to the player.
- Removed the obsolete sequential Case Files flow. The title screen now starts a new roguelike run directly, each cleared floor selects a different venue nonlinearly, and death retries preserve the current floor and loadout instead of reopening selection.
- Split the blood reservoir and `X` Focus into independent native-pixel HUD cards, labels and gauges. Updating either resource can no longer overwrite or overlap the other.
- Rebuilt all twelve venues as readable OTXO-influenced combat boards with four to five broad chambers, short circulation links, generous movement lanes and substantially less maze-like partitioning while retaining each location's original architectural identity.
- Added 32 complete hand-authored tactical room modules with coupled enemy anchors, patrol intent, role order and destructible cover. Eight safe rotations/reflections provide 256 spatial variants; deterministic shuffling prevents repeats within a floor, death retries preserve the arrangement and a new run reshuffles it.
- Added catalog validation for unique IDs, safe room insets, hostile/cover separation and metadata completeness, plus regression coverage for deterministic diversity, HUD separation and nonlinear floor progression.
- Raised ordinary encounter populations to 12-15 mixed hostiles and the expanded Tactical Lab to 17, with one deliberately placed hound per map, mobile two-point patrols and entrance-safe patrol validation.
- Increased readable violence without increasing empowered-ammunition yield: settled ballistic/melee blood is 18% stronger, airborne mist 34% stronger, lethal bursts are broader and injured enemies leave denser persistent one-pixel trails.
- Reserved a full two-cell traversal apron around every authored doorway and relocated two legacy set pieces that overlapped widened thresholds, keeping randomized cover and signature props from blocking an open door.
- Replaced permanent fixed sentries with two-point mobile patrols across all twelve maps. Unentered rooms now keep their visible patrol simulation alive while independently suppressing vision, hearing, tactical orders and attacks, so scouted enemies no longer appear frozen or activate the whole floor.
- Rebalanced the starter Mossberg 590A1 around dense nine-pellet 00 buck: a tighter 6.25-degree pattern, 18 damage per pellet and a longer room-distance falloff band make centre-mass CQB hits reliably decisive while its slow pump cycle remains the handling tradeoff.

- Replaced expanding post-mortem blood pools with a single weapon-authored terminal pixel burst. Penetration, retained/projectile-exit state, energy transfer, range, hit zone and weapon pattern still shape the lethal spray, while settled corpses no longer bleed on a timer.

- Fixed apparent slow siphoning caused by death-pool growth repainting blood during extraction. A successful aimed siphon now consumes that pending growth source, drains ground blood at an arcade-fast rate, and uses a separate hard cap for reserve gain.
- Rebalanced the roguelike siphon into a short mouse-facing cone: hostile blood now leaves the floor quickly while only a small purified fraction enters the reserve, the player flashes crimson during a successful transfer, player wounds/death no longer generate recyclable blood, and traversal no longer creates bloody footprints.
- Blood siphoning now peels visible one-pixel motes from the floor into the player, runs at a combat-ready capped rate, and keeps enemy bodies visible while dormant rooms suspend AI instead of visually spawning hostiles at room boundaries.

## [v1.3.0] - Authentic Modular Armory

- Fixed alerted melee sentries and hounds inheriting firearm-only hold-position doctrine. Close-range actors now abandon authored guard posts, stop at real weapon reach, check the attack before navigation/crowd movement and preserve a committed wind-up through harmless point-blank FOV jitter, eliminating idle rushers and player body-shoving.
- Added map-wide patrol-route validation plus close-contact melee regressions for hound spacing, fixed-sentry pursuit, first wind-up and resolved player damage.
- Completed a full active-mode regression audit across script parsing, all 41 CI scenes and all twelve playable maps. Fixed the orphaned real-time Focus tick, blocked pre-deployment Focus charge loss, and restored real mission elapsed-time accounting.
- Fixed terminal room-run edge cases: enemies killed through a doorway before room entry now count toward completion, completion emits exactly once, cleared-room rewards remain accurate and the final report records the current run instead of reusing stale campaign data.
- Closed empowered-blood duplication paths across penetrating lethal rounds, wall splatter, delayed wounds, detached-limb trails and visual mist. The remaining raw-mass ledger now transfers exactly once to the terminal burst and can never be reused downstream.
- Fixed stationary corpses retaining projectile collision after their overkill window, sparse blood pixels accumulating duplicate stale indices after absorb/redeposit cycles, and scene retries retaining generated actor, environment, light and workbench GPU textures.
- Updated active CI coverage around room runs, blood-resource conservation, entry loadout safety, ragdoll terminal state and current Roguelike HUD/report semantics while retiring obsolete cleanup-only fixtures from the release gate.
- Replaced the visible RMB siphon cone with one-pixel crimson filaments that contract from draining stains into the actor. Radial sparse-pixel collection now visits every coordinate parity, includes wall splatter, and continues intentionally at full reserve without granting overflow, eliminating visible blood that previously appeared impossible to absorb.
- Finalized the blood-powered ammunition contract: ordinary casings and tracers remain gold, RMB-primed rounds use the exact immutable crimson blood palette, and each empowered projectile receives substantially higher damage, penetration, gore and dismemberment pressure with only a restrained impulse increase. Siphonable impact and terminal blood now share a per-trigger raw-mass ledger capped at 72% of the paid blood cost; shotgun pellets divide that ledger and penetrating rounds carry only the unused remainder, making blood-ammo recursion mathematically lossy.
- Replaced the former mandatory combat-to-cleanup mission loop with an OTXO-inspired room-run mode: enemies are partitioned by authored architectural room, dormant encounters remain non-interactive until entered, each occupied room becomes a compact combat stage, and clearing it pauses on a three-choice run upgrade before exploration continues.
- Added a modular crimson resource economy backed by the real sparse 1x1 blood canvas. Holding RMB siphons nearby visible blood at a bounded rate while slowing movement, never fabricates water or UV residue, and still permits simultaneous gunfire.
- Corrected dense-pool siphoning so each absorption tick obeys a hard raw-mass budget and a 4.5 blood-per-second base ceiling; Q/E/R and healing now briefly interrupt siphoning, preventing a nearby large pool from making spent abilities feel immediately free or the reservoir effectively infinite.
- Made LMB contextually fire ordinary ammunition when RMB is released and white-core/crimson-trail empowered ammunition while the blood stance is held. One trigger pull spends blood once even for multi-pellet shotguns, then scales terminal damage and penetration without bypassing the existing ballistic resolver.
- Added contextual RMB+Q Crimson Step, RMB+E Arterial Pulse and RMB+R Coagulate abilities plus B Transfusion healing, with individual blood costs, cooldowns, compact HUD readiness feedback and room-reward upgrades for reservoir, siphon, ballistics, mobility, healing and ritual cadence.
- Added inward-moving one-pixel siphon particles and a native-pixel crimson screen perimeter for the complete stance duration while preserving the global black/white/gray world, persistent crimson gore, doors, destructible or kickable props, ragdolls and ballistic wound trails.
- Added focused regression coverage for blood transfer, shotgun resource accounting, healing, abilities and persistent room upgrades; existing pixel blood, projectile, animation, ragdoll, vertical-slice and all-map door passage tests remain green.
- Reframed the complete application around an OTXO-influenced, original black/white/crimson visual contract. A persistent global presentation layer now covers menus, loadout, HUD, gameplay and results, quantizing all ordinary art to grayscale while preserving deep crimson only for blood, muzzle violence, lethal hazards and crisis feedback.
- Replaced coloured level grading with twelve distinct grayscale exposure signatures, neutral practical lights, pure-black hard shadows and monochrome nightclub volume beams; material pattern, architecture and luminance now carry each venue's identity instead of hue.
- Reworked the native-pixel post stack with stronger chiaroscuro, fine 64-step grayscale separation, restrained 12 Hz flat-field grain, edge-aware scanlines and a claustrophobic vignette. Focus remains readable through monochrome exposure and border breathing without introducing forbidden cyan or purple.
- Protected bright one-pixel glyphs and icons from scanline/grain attenuation and mapped former UI accent saturation to neutral value before grayscale conversion, retaining crisp text hierarchy throughout the global noir presentation layer.
- Added four-neighbour native-pixel edge protection across the complete frame, disabled soft environment glow and hard-disabled registration displacement so isolated 1x1 material marks, silhouettes, blood droplets and glyph stems remain spatially exact.
- Changed gunfire to a crimson/white-hot muzzle signature, wall impacts to dense white sparks plus gray concrete dust and door slams to larger white/gray splinter bursts, preserving the permanent red blood layer as the primary record of violence.
- Rebuilt the heaviest pixel-rendering paths without reducing their visual density: nightclub volumetric beams and practical-light halos now rasterize into nearest-filtered textures instead of issuing thousands of per-pixel draw commands every refresh.
- Consolidated every simultaneous impact spray into one room-wide adaptive MultiMesh blood-mist renderer, retaining directional trails, wall stops and permanent one-pixel deposits while eliminating per-hit scene and draw-list churn.
- Reworked blood and environmental-liquid canvases around sparse dirty-pixel queues with bounded texture uploads per frame; large spills, pools, wet cleanup and conductive water no longer synchronize every active chunk on the same tick.
- Batched the physical pressure-washer stream through a single MultiMesh draw submission, kept its packet simulation and cleaning strength intact, and disabled settled shell-casing callbacks once their motion is complete.
- Added distance/reactivity-aware visual-rig cadence, a fixed-ring performance sampler, quarter-second debug-HUD reporting and live draw-call/render-object/video-memory peaks so optimization work can be measured without changing AI decisions or gameplay timing.
- Rebuilt mission one's nightclub from a real venue program: a controlled entrance/coat-check sequence opens onto a broad dance floor, with toilets isolated to the west, a DJ stage and VIP/cocktail band to the north, and a main bar plus independent back-bar service route to the east. Restrained magenta/teal practical lighting, sparse wayfinding, believable fixtures, updated patrols, music-masked acoustics and nightclub-specific cleanup surfaces replace the former crude colour maze.
- Re-rendered that nightclub as two native-resolution material fields—floor plus structure—where every visible world texel is an independently selected 1x1 pixel. Hand-authored dance-plank, LED-stage, VIP-fabric, terrazzo, glazed-tile, glass, painted-wall and bar-mosaic palettes eliminate uniform 2x2 blocks and expose more than two dozen controlled colours without adding per-pixel nodes.
- Added a pixel-native lighting stack: every room light now uses a four-band 128px nearest-neighbour mask, nightclub walls are greedily merged into hard-edge LightOccluder2D geometry, door leaves carry rotating occluders, and three low-alpha 1x1 dithered stage/bar beams ray-stop at architecture to simulate restrained 2D volumetric light without blur.
- Corrected the first lighting pass after playtest: PointLight2D illumination now has a visible 1x1 white/coloured practical emitter and sparse pixel halo, while the club material distribution uses roughly eighty-percent muted dark fibres, wood, stone and tile with rare localized neon reflections instead of uniformly saturated confetti noise.
- Added a Noita-inspired world-space material canvas that resolves every visible floor texel from its material and absolute coordinate. Grain, aggregate, fibres, grout chips, timber seams, brick bonds, terrazzo fragments and mosaic pieces now continue across tile boundaries instead of visibly repeating as large 8x8 color stamps, while generation remains a single static texture with no per-pixel nodes or frame simulation.
- Expanded the procedural environment atlas from 32 to 40 strict 8x8 tiles while preserving a one-world-pixel texel grid: terrazzo, blue ceramic, gold carpet, rubber service grid, white ceramic, neon mosaic, dark feature walls and industrial service walls now add venue-specific material depth without mixed pixel sizes.
- Repainted the core concrete, timber, carpet, checker, grass, plank, cream, stage, brick, bathroom and wall families with four-to-seven coordinated highlight, midtone, shadow and wear colors; every mark remains hand-placed at native one-pixel resolution and nearest filtered.
- Reassigned all twelve architectural plans by actual room use—wet rooms, kitchens, laboratories, loading bays, VIP lounges, offices, public circulation and production spaces now read through coherent floor finishes, while exterior and interior walls use separate bright, nightlife and service palettes.
- Preserved systemic cleanup behavior across the richer finishes with distinct carpet absorption, timber grain, glazed-tile spread and rubber-floor response, and added regression coverage for atlas dimensions, opacity, transparency and minimum per-material color complexity.
- Replanned all twelve campaign maps on a shared half-metre architectural grid: human-scale circulation, six-to-nine named functional rooms per venue, believable public/service zoning, clearer thresholds, additional internal doors, room-specific floor finishes and distinct tactical circulation now replace broad rectangular arena partitions.
- Reauthored every venue's wall topology around its real-world purpose—club, restaurant, laboratory, harbor warehouse, motel, penthouse, cold store, casino, police archive, slaughterhouse, broadcast station and finale club—while preserving exterior arrival, local exploration framing, AI acoustic sectors, destructible furnishing, cleanup access and full doorway traversal.
- Added architectural regression coverage for scale, minimum room count, unique functional identities, room dimensions and building-envelope validity alongside the existing all-enemy reachability, door, patrol, visibility and acoustic checks.
- Rebuilt the in-mission loadout overlay around a compact three-zone grid and a dedicated framed footer; hardpoints, aggregate stats and the primary Deploy action no longer overlap or escape the panel at the native 320x180 viewport.
- Replaced universal one-shot firearms with a centralized high-lethality vitality model inspired by Intravenous II: enemies now occupy 75/85/95 HP experience tiers, hounds use 65 HP and the player uses 115 HP.
- Added weapon-class and calibre damage, head/torso/limb multipliers, effective-range falloff, minimum terminal damage, alert-aware stealth criticals and durable armour whose reduction changes with projectile penetration.
- Differentiated player protection by field kit and gave heavy enemies readable body armour, while preserving decisive headshots, close shotgun lethality, executions and unaware takedowns.
- Added a compact lower-left vitality/armour HUD, wound-scaled blood feedback and focused regression coverage.
- Rebuilt firearm blood around resolved terminal ballistics: armour stops suppress exit spray, well-matched penetration transfers the most tissue energy, and extreme over-penetration no longer scales blood monotonically upward.
- Added persistent non-lethal wounds with calibre-, hit-zone-, range-, armour- and penetration-sensitive bleeding rates; moving casualties now leave sparse one-pixel trails while gradual blood loss cannot silently kill an actor without a resolved death context.
- Kept persistent wounds independent from tactical decision-making: a hit still interrupts and staggers, but no longer injects a forced-cover order that can oscillate against the established attack, flank and investigation state machine.
- Expanded the complete regression suite to 48 scenes with terminal-wound, armour-stop, over-penetration, moving-blood-trail, retry-loadout and fair-fire camera coverage.
- Expanded ordinary mouse look-ahead and retained a farther deliberate look mode; ranged hostiles may track and reposition beyond the frame but cannot complete a shot until their body enters the player's readable camera safe area.
- Made loadout staging a once-per-mission-entry decision: immediate death retries reuse the chosen configuration, while later fresh entries show the field bench again.
- Closed the pre-deployment ESC exploit by keeping the loadout modal, player controls, hostile simulation and pause state locked until the player explicitly confirms deployment.
- Rebuilt living-enemy hit confirmation around the existing lifecycle rig: a high-contrast warm-white impact frame resolves into a short crimson flesh/head flash, armour uses a cold cyan response, rapid fire reinforces instead of restarting the bright phase, and lethal hits hand off directly to death presentation.
- Added compact reticle and procedural impact-audio confirmation for flesh, armour and lethal hits while preserving hit-zone impulses, restrained hit-stop and the uncluttered combat HUD.
- Added 64 individually tuned real-world firearm platforms: eight each across handgun, PDW, SMG, shotgun, carbine, DMR, sniper and machine-gun classes.
- Added dedicated Armory and Gunsmith screens with primary/secondary configuration, unlock status, platform specifications, signature traits, build comparison and a direct tactical-range route.
- Rebuilt pre-mission weapon configuration as a contained field-bench overlay inspired by established tactical loadout tables: the live entrance remains visible around a dimmed 312x170 panel, with five pictured presets and five pictured inventory slots at left, a paginated image-led platform roster with six icon stats at right, pictured modification hardpoints below, and segmented signature/mobility beside confirmation.
- Added clearly named firearm category filters to the field bench and a direct right-click modification workflow for equipped and available weapons.
- Expanded every firearm class with two differentiated real platforms and made the complete roster visible as equipped, available or locked requisitions instead of hiding future unlocks.
- Added a dedicated in-mission modification workbench with a large live weapon silhouette, five pictured hardpoints, six comparison stats, paginated compatible-part cards, staged fitting/removal and safe return to the loadout bench.
- Added unique 32x20 pixel pictures for every compatible attachment; similar suppressors, optics, grips, magazines, ammunition and stocks now carry distinct geometry and maker marks instead of sharing a generic slot glyph.
- Added complete mouse and keyboard operation for the field bench, including direct platform rows, preset loading, primary/secondary switching, attachment browsing, confirmation, safe cancellation and exit-to-rearm behavior at the physical case.
- Added strict nearest-filtered 64x24 firearm renders and 16x16 attachment icons generated from each platform, feed system, receiver geometry, real length and installed parts; the preset, Armory and Gunsmith screens now reuse the same visual language instead of relying on text walls.
- Added five mutually exclusive attachment slots and compatibility rules for calibre, muzzle interface, optic mount, accessory rail, magazine family and stock/grip interface, including integral-suppressor exclusions.
- Added real accessory families for suppressors, brakes, chokes, red dots, magnified optics, lights, lasers, foregrips, bipods, magazines, ammunition and stocks.
- Connected attachment results to live accuracy, recoil, heat growth, aim speed, movement, reload, capacity, projectile velocity, penetration, property damage, report, muzzle flash, cleanup burden, camera look-ahead and physical weapon length.
- Preserved weapon builds through custom mission issue, enemy equipment, world pickups, throws and recovery; added schema-7 save migration and stable aliases for the four legacy weapon IDs.
- Updated procedural silhouettes, muzzle positions, projectile trails, audio profiles, hit response and corpse damage families for the expanded platform classes.
- Expanded the workshop with a Gunsmith Bench route and added regression coverage for roster completeness, compatibility rejection, build persistence, spatial entry staging, visual-icon completeness, UI fit and runtime application; all 46 test scenes pass.
...
- Moved finite combat Focus to X/left shoulder and made combat RMB/left trigger a hold-to-target context mode; cleanup RMB remains ultraviolet inspection.
- Added readable Call of Duty-inspired hit-marker tiers for flesh, armour, head and lethal impacts, plus context brackets for hostiles, downed targets, cameras and destructible objects.
