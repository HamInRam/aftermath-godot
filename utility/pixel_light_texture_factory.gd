...
static func create_texture() -> ImageTexture:
	...
	for y in SIZE:
		for x in SIZE:
			...
			# Native 1px illumination with continuous intensity, not four stepped
			# discs and ordered holes that read as white contour lines on the floor.
			var alpha := 1.0 - smoothstep(0.0, 1.0, ratio)
