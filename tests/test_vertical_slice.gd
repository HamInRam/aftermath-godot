...
func _run() -> void:
	...
	_expect(level.combat_focus_active and level.combat_focus_charges == level.COMBAT_FOCUS_MAX_CHARGES - 1, "one Focus press should spend exactly one finite charge")
	...
	_expect(is_equal_approx(focused_enemy.combat_time_scale, level.COMBAT_FOCUS_TIME_SCALE) and is_equal_approx(focused_enemy.gun.combat_time_scale, level.COMBAT_FOCUS_TIME_SCALE) and is_equal_approx(focused_enemy.move_speed, unfocused_enemy_speed * level.COMBAT_FOCUS_TIME_SCALE), "combat Focus should slow enemy decisions, movement and weapon cadence locally")
	...
	_expect(not hostile_bullets.is_empty() and is_equal_approx(hostile_bullets[0].combat_time_scale, level.COMBAT_FOCUS_TIME_SCALE), "hostile rounds spawned during Focus should inherit local slowdown")
	Input.action_release("combat_focus")
	level._update_combat_focus(1.0 / 60.0)
	_expect(level.combat_focus_active and is_equal_approx(focused_enemy.combat_time_scale, level.COMBAT_FOCUS_TIME_SCALE), "Focus should run for its fixed duration after RMB is released")
	...
	_expect(level.run_over and level.final_score > 0, "room completion should produce a combat report")
	...
	_expect(not level.player.controls_enabled, "floor report should stop gameplay input")
