...
func _run() -> void:
	...
	_expect(level.combat_focus_active and level.combat_focus_energy < 1.0 and is_equal_approx(level.combat_feedback.base_time_scale, level.COMBAT_FOCUS_TIME_SCALE), "holding combat Focus should spend a finite meter and set the composable slow-motion scale")
	Input.action_release("combat_focus")
	level._update_combat_focus(1.0 / 60.0)
	_expect(not level.combat_focus_active and is_equal_approx(level.combat_feedback.base_time_scale, 1.0), "releasing Focus should immediately restore normal combat time")
