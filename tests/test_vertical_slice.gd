...
func _run() -> void:
	...
	_expect(level.combat_focus_active and level.combat_focus_energy < 1.0, "holding combat Focus should spend a finite meter")
	...
	level._update_combat_focus(1.0 / 60.0)
	_expect(not level.combat_focus_active and is_equal_approx(focused_enemy.combat_time_scale, 1.0) and is_equal_approx(focused_enemy.move_speed, unfocused_enemy_speed) and is_equal_approx(hostile_bullets[0].combat_time_scale, 1.0), "releasing Focus should immediately restore hostile simulation speed")
