...
func _ready() -> void:
	...
	_expect(liquids.get_debug_jet_count() == 20, "the stock focused nozzle should emit four readable pixel-water packets per pulse")
	_expect(float(liquids.jet_particles[0].radius) >= 1.6, "the stock focused jet should have a forgiving physical impact footprint")
	_expect(liquids.get_debug_pressure_stability() >= 0.75 and int(liquids.jet_particles[-1].power) > 9, "holding a steady target for roughly a quarter second should build a twenty-five-percent pressure bonus")
	...
	_expect(float(liquids.jet_particles[0].radius) >= 2.0, "washer level two should enlarge the real near-impact radius")
	liquids.jet_particles.clear()
