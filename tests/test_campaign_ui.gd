...
func _ready() -> void:
	...
	_expect(compact_hud.resource_backplate.get_rect().encloses(compact_hud.ammo_meter.get_rect()) and compact_hud.ammo_meter.size == Vector2(72, 5), "magazine gauge must remain pixel-sharp inside the current ammunition card")
