...
func _run() -> void:
	...
	var blood_test_position := tile_world.map_authored_position(Vector2(192, 100))
	level.blood_system.spawn_micro_drop(blood_test_position, 0.8, Vector2.RIGHT)
	await get_tree().process_frame
	var projected_floor := tile_world.get_nearest_walkable_position(blood_test_position)
	...
	level.blood_system._spawn_ground_splatter(blood_test_position, Vector2.RIGHT, 1.0, "fan", 0.7)
	await get_tree().process_frame
