...
func _ready() -> void:
	...
	_expect(prop.state == DestructibleProp.PropState.DAMAGED and prop.erosion != null and prop.erosion.remaining() > 0, "a projectile must chip a surviving prop instead of deleting it")
	prop.receive_door_impact(Vector2.RIGHT, 4.0)
	...
	_expect(not prop.has_method("interact"), "destroyed sources must not expose retired cleanup actions")
	_expect(is_instance_valid(prop.active_hazard), "destroyed plumbing retains its combat hazard")
