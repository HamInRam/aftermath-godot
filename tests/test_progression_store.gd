...
func _ready() -> void:
	...
	var rogue_first := store.begin_roguelike_run()
	_expect(rogue_first != null and store.get_roguelike_floor() == 1, "a new roguelike run should choose a floor without a linear mission selection")
	_expect(store.begin_next_roguelike_floor() == null and store.get_roguelike_floor() == 1, "uncleared floors cannot be skipped")
	store.run_session.finish_floor(1, {"rooms_cleared": 4}, 1000, {})
	var rogue_second := store.begin_next_roguelike_floor()
	_expect(rogue_second != null and rogue_second.mission_id != rogue_first.mission_id and store.get_roguelike_floor() == 2, "the next roguelike floor should be non-linear and avoid an immediate venue repeat")
	_expect(store.begin_next_roguelike_floor() == null and store.get_roguelike_floor() == 2, "repeated continuation cannot skip an unplayed floor")
	store.reset_progress(true)
	...
	_expect(store.cycle_kit() == "custom" and store.cycle_kit() == "balanced", "field-kit cycling should wrap through custom configuration into the authored kit order")
