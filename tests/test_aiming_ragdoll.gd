...
func _ready() -> void:
	...
	var packaging_corpse := CORPSE_SCENE.instantiate()
	add_child(packaging_corpse)
	packaging_corpse.setup(0.0, Vector2.RIGHT, 45.0, 1.5, "firearm", "torso", "shotgun")
	for frame in 18: packaging_corpse.ragdoll._physics_process(1.0 / 60.0)
	var visible_body_position: Vector2 = packaging_corpse.get_interaction_position()
	packaging_corpse.apply_cleanup_tool("body_bag")
	_expect(packaging_corpse.global_position.distance_to(visible_body_position) <= 0.05, "body bags must replace the corpse at its visible ragdoll landing position, not its stale death origin")
